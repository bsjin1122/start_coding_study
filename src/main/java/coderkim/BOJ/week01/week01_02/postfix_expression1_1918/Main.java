...
/**
  ...
  연산자가 피연산자 가운데 위치하는 중위 표기법(일반적으로 우리가 쓰는 방법이다), 연산자가 피연산자 앞에 위치하는 전위 표기법(prefix notation),
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < infix.length(); i++) {
            ...
            if (Character.isLetter(c)) {
                // 피연산자 (알파벳)인 경우, 후위 표기식에 추가
                postfixExpression.append(c);
            } else if (c == '(') {
                // 여는 괄호인 경우, 스택에 넣습니다.
                stack.push(c);
            } else if (c == ')') {
                // 닫는 괄호인 경우, 여는 괄호를 만날 때까지 스택에서 연산자를 꺼내 후위 표기식에 추가
                ...
                // 연산자인 경우 스택에서 우선 순위가 높거나 같은 연산자를 모두 꺼내 후위 표기식에 추가
